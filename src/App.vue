...
import { isPublicShareLaunch, redirectShareViewOnDirectEntry } from '@/utils/session'

const USE_MOCK = import.meta.env.VITE_USE_MOCK === 'true'

onLaunch(async (options) => {
  applyInnerAudioOutputOptions()
  const shareLaunch = isPublicShareLaunch(options)
  // Public share entry: skip auth side-effects so stale tokens do not reLaunch to login.
  if (!shareLaunch) {
    void preloadSecuritySettings()
  }
  const token = uni.getStorageSync('mini_token')
  if (USE_MOCK || !token || shareLaunch) return
...
onShow(() => {
  redirectShareViewOnDirectEntry()
})
</script>
