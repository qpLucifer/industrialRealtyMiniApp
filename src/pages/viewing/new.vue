...
function syncPickersFromCode() {
  if (propCode.value) {
    const p = properties.value.find((x) => x.id === propCode.value)
    if (p) {
      propertyTitle.value = `${p.id} · ${p.title}`
      const i = properties.value.findIndex((x) => x.id === propCode.value)
...
onLoad(async (q) => {
  defaultSlot()
  if (q?.propId) {
    propCode.value = String(q.propId)
...
function onPropertyPick(e: { detail: { value: string } }) {
  ...
  const p = properties.value[propertyIdx.value]
  propCode.value = p?.id || ''
  propertyTitle.value = p ? `${p.id} · ${p.title}` : ''
}
