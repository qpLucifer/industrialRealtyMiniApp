...
function propLabel(v: ViewingListItem) {
  const title = String(v.propertyTitle || '').trim()
...
function goDetail(v: ViewingListItem) {
  uni.navigateTo({ url: `/pages/viewing/detail?id=${v.id}` })
}
    ...
    <view class="page-frame screen active screen--sub">
      ...
      <view class="list-page-head page-scroll__inner viewing-page">
        <text class="viewing-page__hint">
          {{ weekOnly ? '仅显示近 7 天带看 · ' : '' }}点击进入详情 · 进行中会高亮
        </text>
      ...
      >
        <template #item="{ item: v }">
          <view
            ...
            @tap="goDetail(v as ViewingListItem)"
          >
